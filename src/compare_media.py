...
def section_profile(df: pd.DataFrame, res: dict, threshold: int) -> None:
    ...
    say("")
    say("  ※ 보유율 자체의 차이는 매체의 성격 차이에서 기인한다. 이하에서는 보유 상태가 동일한")
    say("    건에 한정하여 비교한다.")
    res["보유율"] = hold


def section_same_profile(df: pd.DataFrame, res: dict) -> dict:
    head("2. 동일한 내용정보 보유 시 매체별 등급 차이")
    say("명칭이 동일한 4개 항목의 보유 조합이 완전히 일치하는 건에 한정하여 비교한다.")
    say("양측 모두 표본이 50건 이상인 조합만 제시한다.")
    ...
    if diffs:
        ...
        say(f"    게임물의 제한 비율이 높은 조합 {harsher}개 / "
            f"영상물의 제한 비율이 높은 조합 {len(diffs) - harsher}개")
...
def section_item_effect(df: pd.DataFrame, res: dict) -> None:
    head("3. 동일 항목의 매체별 영향 차이")
    say("항목을 하나만 보유한 건(단독 보유)을 대상으로 비교한다. 다른 항목이 혼재되지 않으므로")
    say("해당 항목 단독의 영향을 확인할 수 있다.")
...
def section_structure(res: dict) -> None:
    ...
    say("     일치율 99.90%(2017년 5월 이후 100.000%)이며, 4단계 분석에서 확인하였다.")
    say("     항목별로 1~5단계가 부여되며, 그중 최고 단계가 그대로 관람등급이 된다.")
    say("     따라서 어떤 항목인지보다 어느 단계까지 도달하였는지가 등급을 결정한다.")
    say("")
    say("  [게임물]  수준 개념이 존재하지 않으며, 해당 항목의 명칭만 나열된다.")
    say("     내용정보가 공란인 13,993건의 청소년이용불가 비율은 0.06%이다(이 중 99.7%가 전체이용가).")
    say("     즉 공란은 미기재가 아니라 해당 요소가 없음을 의미한다.")
    say("     반면 항목이 하나만 부여되어도 청소년이용불가 비율은 57.3%로 상승한다.")
    say("     또한 그 비율은 항목별로 큰 차이를 보인다(사행성 단독 97.1%, 폭력성 단독 11.1%).")
    say("")
    say("  ▶ 종합하면 다음과 같다.")
    say("     영상물은 수준을 기준으로, 게임물은 항목의 종류를 기준으로 등급을 결정한다.")
    say("     따라서 '동일한 내용 수준'이라는 개념이 두 매체에서 같은 의미를 갖지 않는다.")
    say("     2절과 3절의 비교는 영상물을 이진화하여 대응시킨 결과이므로 그 한계를 전제로")
    say("     해석하여야 한다.")
...
<b>최고값</b>이 그대로 관람등급이 되는 항등식 관계(99.90%)이며, 게임물은 수준 개념 없이 해당 항목의
<b>종류</b>만 제시된다. 따라서 '동일한 내용 수준'이라는 개념이 두 매체에서 같은 의미를 갖지 않는다.
이하의 비율 비교는 영상물을 이진화하여 대응시킨 결과이므로 그 한계를 전제로 해석하여야 한다.</div>
<pre>{body}</pre>
</div></body></html>"""
...
def main() -> None:
    ...
    for threshold in (3, 4):
        ...
        if threshold == 3:
            ...
            say("")
            say("※ 비교 대상 외 항목을 제외하는 이유는 다음과 같다. 제외하지 않을 경우 해당 항목에")
            say("  의하여 등급이 결정된 건이 '항목 미보유' 구간에 포함된다. 게임물의 사행성이 대표적")
            say("  사례로, 사행성은 단독 보유만으로 청소년이용불가 비율이 97.1%에 이르나 비교 대상")
            say("  4개 항목에 포함되지 않는다. 제외하지 않으면 '(없음)' 구간의 게임물 비율이 28.3%로")
            say("  과대 산출된다. 영상물에서도 대칭적으로 주제·대사·모방위험을 제외하였다.")
        sub: dict = {}
    ...
    head("5. 이진화 기준에 따른 결과의 안정성")
    say("영상물의 이진화 기준을 3단계와 4단계로 달리하여 동일한 분석을 두 차례 수행하였다.")
    say("기준 변경에 따라 결론이 달라진다면 해당 결론은 사용할 수 없다.")
    say("")
    ...
    if len(vals) == 2 and vals[0] * vals[1] < 0:
        say("")
        say("  ▶ 방향이 반대로 나타난다. 다만 이를 기준에 따라 결론이 달라지는 것으로 해석하여서는")
        say("    안 된다. 4단계 기준 자체가 성립하지 않기 때문이다.")
        say("")
        say("    영상물은 등급이 내용정보의 최고값과 동일하며, 4단계가 곧 청소년관람불가에 해당한다")
        say("    (4.7절 항등식). 따라서 4단계 이상을 보유로 간주하여 이진화하면 영상물은 정의상")
        say("    100%로 산출된다. 위 표에서도 영상물이 99.8~100.0%로 나타난다. 이는 비교가 아니라")
        say("    정의를 재확인한 것에 해당한다.")
        say("")
        say("    ▶ 따라서 매체 비교에 사용할 수 있는 기준은 3단계에 한정된다.")
        say(f"      해당 기준에서는 비교 가능한 9개 조합 전부에서 게임물의 제한 비율이 높게")
        say(f"      나타났다(평균 {vals[0] * 100:+.1f}%p). 동일 항목을 단독 보유한 경우에도")
        say("      선정성 12.6%→55.8%, 폭력성 0.8%→11.0%, 약물 3.1%→25.0%, 공포 0.7%→5.8%로")
        say("      모두 게임물의 값이 높다.")
        say("")
        say("    한계는 다음과 같다. 3단계라는 절단 기준 자체에 실증적 근거가 있는 것은 아니다.")
        say("    영상물의 5단계 체계를 게임물의 보유·미보유 체계에 대응시키려면 특정 지점에서")
        say("    절단이 필요하나, 그 지점은 데이터로부터 도출되지 않는다. 게임물의 판정이 더")
        say("    엄격하다는 결론은 이 절단 기준을 전제로 할 때에만 성립한다.")
    elif len(vals) == 2:
        say("")
        say("  ▶ 두 기준에서 방향이 동일하다. 본 비교는 이진화 기준에 좌우되지 않는다.")
