...
def s1(df: pd.DataFrame, res: dict) -> None:
    head("1. 등급 결정 규칙 — 관람등급은 내용정보의 최고값과 동일하다")
    say("내용정보 7개 항목의 최고 단계를 관람등급에 대응시킨 결과를 확인한다.")
    ...
    say("")
    say("  ▶ 2017년 5월부터 적용된 등급표기 구간에서는 예외가 확인되지 않는다. 불일치 건은 모두")
    say("    그 이전 구간에 해당하며, 대부분 최고값이 5단계(매우높음)임에도 제한상영가가 아니라")
    say("    청소년관람불가로 결정된 사례이다.")
    ...
    say("")
    say("  ※ 이것이 뜻하는 것")
    say("    내용정보로 등급을 예측하는 모형에서는 AUC가 1.000으로 산출된다. 이는 모형의 성능이")
    say("    아니라 동일한 값을 두 번 사용한 결과이다. '폭력성이 높으면 청소년이용불가가 많다'는")
    say("    진술은 발견이 아니라 정의에 해당한다. 따라서 분석의 초점을 전환할 필요가 있다.")
...
def s2(df: pd.DataFrame, res: dict) -> None:
    head("2. '조합 효과' 결과의 발생 원인 — 이진화에 따른 착시")
    say("3단계 이상을 보유로 처리하면 3·4·5단계 간의 차이가 소거된다.")
    say("그러나 등급을 구분하는 것은 정확히 그 차이이다(4단계 이상이어야 청소년관람불가).")
    say("그 결과 보유 항목 수가 동일함에도 결과가 다르게 나타나는 착시가 발생한다.")

    nd = df[~df["is_adult"]]

    say("")
    say("  [항목 수를 고정하고 항목 구성만 달리한 경우의 차이] 종전 분석 결과")
    eff = {}
    for i in range(1, 8):
        ds = []
        for n in range(1, 8):
    ...
    say("")
    say("  [항목별 3단계 이상 대비 4단계 이상 도달 비율]")
    ...
    if len(pairs) >= 3:
        ...
        say(f"  두 표의 상관 {r:+.2f}")
        if r > 0.4:
            say("    → 4단계 이상 도달 비율이 높은 항목일수록 등급을 상승시키는 것처럼 나타난다.")
            say("      즉 §2의 첫 번째 표는 심의 판단의 차이가 아니라 항목별 단계 분포의 차이를")
            say("      반영한 것이다.")
        else:
            say("    → 단계 분포만으로는 충분히 설명되지 않으며, 항목 간 동반 상승(상관 구조)도")
            say("      함께 작용한다. 어느 경우든 조합 자체가 등급을 결정한다는 해석은 성립하지 않는다.")

    say("")
    say("  ▶ 정정 사항. 종전에 '항목 수가 동일하여도 조합에 따라 12.7%~66.9%로 갈린다'고")
    say("    기술하였으나, 이는 최고값 규칙을 이진화로 처리한 데 따른 결과이므로 분석 결과로")
    say("    사용할 수 없다.")
...
def s3(df: pd.DataFrame, res: dict) -> None:
    head("3. 등급 상향을 유발하는 항목 — 종별·시기별 차이")
    say("등급이 최고값과 동일하다면 남는 논점은 어떤 항목이 그 최고값을 차지하는가이다.")
    say("결정사유(rtCoreHarmRsnNm) 필드가 해당 항목을 명칭으로 제공한다.")
    ...
    yrs = has["year"].dropna()
    if len(yrs):
        say(f"  기록 구간 {int(yrs.min())} ~ {int(yrs.max())} — 그 이전 건에는 해당 필드가 공란이다.")
        say("  따라서 본 절의 결과는 전 기간이 아니라 최근 구간에 한정하여 해석하여야 한다.")
    ...
    say("")
    say("  ▶ 종별에 따라 등급 상향을 유발하는 항목이 뚜렷하게 구분된다. 이는 최고값 규칙에서")
    say("    자동으로 도출되는 결과가 아니라 콘텐츠 성격의 차이를 보여주는 결과이다.")
...
def s4(df: pd.DataFrame, res: dict) -> dict:
    head("4. 신청등급과 결정등급의 차이 — 최고값 규칙으로 설명되지 않는 영역")
    say("내용정보와 결정등급은 동일한 값이므로 상호 설명력이 없으나 신청등급은 성격이 다르다.")
    say("신청등급은 신청인이 자체적으로 판단한 값이고, 결정등급은 위원회가 결정한 값이다.")
    say("양자가 상이한 지점이 본 자료에서 판단이 개입하는 유일한 영역이다.")
    ...
    say("")
    say("  ※ 유의 사항. 결정등급이 최고값과 동일하므로 상향은 '최고값 > 신청등급'과 동치이다.")
    say("    따라서 신청등급을 설명 변수로 포함하면 다시 항등식이 되므로 포함하지 않았다.")
    say("    결과적으로 본 모형이 측정하는 것은 신청인이 어느 지점에서 등급을 낮게 예측하는가이다.")
    ...
    if auc >= 0.7:
        say("    → 설명력이 상당한 수준이다. 위원회의 상향 판단에 일정한 경향이 존재함을 의미한다.")
    elif auc >= 0.6:
        say("    → 일정 수준 설명된다. 다만 설명되지 않는 부분이 커서 개별 판단의 여지가 넓다.")
    else:
        say("    → 설명력이 거의 없다. 확보된 변수로는 위원회의 상향 판단을 설명하지 못한다.")
    ...
    say("")
    say("    [오즈비] 1보다 큰 경우 상향 조정 가능성이 높아진다. 내용정보 항목만 제시")
...
def s5(df: pd.DataFrame, res: dict) -> None:
    ...
    say("     내용정보로 등급을 설명하는 분석은 모두 동어반복에 해당한다. PRD 5.3 함정 1에서")
    say("     제시한 상황이 경향이 아니라 항등식으로 확인되었으며, 이 자체가 분석 결과이다.")
    say("")
