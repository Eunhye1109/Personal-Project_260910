...
1) 내용정보는 이름이 같은 것만 같은 칸에 넣는다
   선정성·폭력성·공포·약물 네 개만 두 매체가 같은 이름을 쓴다.
   대사(영상) ↔ 언어(게임), 주제·모방위험(영상) ↔ 범죄·사행성(게임)은
   재는 대상이 달라 통합하지 않고 각자의 칸에 남긴다. 합치면 없는 값을 지어내는 셈이다.

2) 영상물의 1~5단계는 3단계 이상을 '보유'로 본다
   게임위는 수준 없이 항목 이름만 주므로 보유/미보유로 맞춰야 비교가 된다.
   4단계 기준은 쓸 수 없다 — 영상물은 등급이 내용정보의 최고값이라(항등식 99.9%)
   4단계 이상 = 청소년관람불가가 정의상 성립해 버린다. 원 단계값도 `단계_*`에 그대로 남긴다.

3) 비교에서 빼야 할 건에는 표시만 하고 지우지 않는다
   `비교대상밖_보유` = 비교 대상 4개 밖의 항목이 붙은 건.
   `비교표본` = 그 건과 등급취소·성인물·등급 없는 건을 뺀 나머지.
   지우지 않는 이유는, 통합본은 분석마다 쓰임이 달라서다. 거르는 것은 쓰는 쪽이 정한다.
...
def blank(n: int, names: list[str], df: pd.DataFrame) -> None:
    """해당 매체에 없는 항목 칸은 0이 아니라 결측으로 둔다. 0으로 두면 '없다'가 되어 버린다."""
    for name in names:
...
def dictionary_html(df: pd.DataFrame, meta: dict) -> Path:
    desc = {
        ...
        "등급명": "기관 표기 그대로의 등급",
        "등급연령": "등급을 이용 가능 연령(0/12/15/18)으로 환산",
        "청소년이용제한": "게임물 청소년이용불가 / 영상물 청소년관람불가·제한관람가",
        "분류일자": "등급분류 결정일", "분류연도": "분류일자의 연", "분류월": "분류일자의 연-월",
        "항목수": "보유한 내용정보 항목 수(해당 매체에 있는 항목 기준)",
        "비교대상밖_보유": "공통 4항목 밖의 항목(게임=언어·범죄·사행성 / 영상=주제·대사·모방위험) 보유 여부",
        "비교표본": "매체 간 비교에 쓸 수 있는 건인지 — 비교대상밖·등급취소·성인물·등급없음 제외",
        "등급취소": "게임물 등급취소 여부",
        "원본": "이 행이 나온 정제본 파일명",
    }
    rows = []
    for c in df.columns:
        if c.startswith("내용_"):
            d = "보유=1 / 미보유=0 / 해당 매체에 없는 항목=결측"
            if c[3:] in COMPARABLE + KMRB_ONLY:
                d += f" (영상물은 {THRESHOLD}단계 이상을 보유로 본 값)"
        elif c.startswith("단계_"):
            d = "영상물 원 단계값 1~5 (게임물은 결측 — 수준이라는 개념이 없다)"
        else:
            d = desc.get(c, "")
...
<div class="key"><b>세 갈래를 한 표로 붙였다.</b> 게임물 위원회분류 {meta['게임물_위원회']:,}건,
게임물 자체등급분류 {meta['게임물_자체']:,}건, 영상물 {meta['영상물']:,}건.
<b>이름이 같은 내용정보 4개(선정성·폭력성·공포·약물)만 같은 칸에 넣었고</b>, 나머지는 각 매체의
칸에 그대로 뒀다. 영상물의 1~5단계는 {THRESHOLD}단계 이상을 보유로 본 이진값과 원 단계값을 함께 싣는다.
거를 건은 지우지 않고 <code>비교표본</code> 칸에 표시만 했다 — 무엇을 거를지는 분석마다 다르기 때문이다.
그 칸이 참인 건은 {meta['비교표본']:,}건이다.</div>
<h2>열 정의</h2>
