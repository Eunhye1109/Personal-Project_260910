...
<section>
  ...
  <div class="finds">
    <div class="find">
      <span class="tag">등급 결정 규칙</span>
      <h3>관람등급은 내용정보의 가장 높은 단계와 같다</h3>
      <p>내용정보 7개 항목 중 가장 높은 단계를 그대로 옮기면 관람등급이 된다.
      {v['n']:,}건 가운데 {v['rule_rate']*100:.2f}%가 이 규칙을 따르고,
      표기가 바뀐 2017년 5월 이후 {grade_n:,}건은 어긋난 건이 하나도 없다.
      즉 내용정보로 등급을 설명하는 것은 같은 값을 두 번 쓰는 셈이다.</p>
    </div>
    <div class="find">
      <span class="tag">구조 변화</span>
      <h3>청소년관람불가가 {y0[2][3]:.0f}%에서 {y1[2][3]:.0f}%로</h3>
      <p>{y0[0]}년에는 등급분류된 비디오물의 {y0[2][3]:.0f}%가 청소년관람불가였다.
      {y1[0]}년에는 {y1[2][3]:.0f}%다. 심의가 관대해졌다기보다
      <strong>등급분류를 신청하는 대상 자체가 바뀐 것</strong>으로 보인다.</p>
    </div>
    <div class="find">
      <span class="tag">표본 구성</span>
      <h3>성인물이 {v['adult_share']*100:.0f}%를 차지한다</h3>
      <p>종별에서 성인물 비중이 크다. 성인물은 사실상 전건이 청소년관람불가라,
      이를 통제하지 않으면 내용요소의 효과가 아니라 성인물의 비중 변화를 보게 된다.
      아래 해석에서 계속 문제가 되는 지점이다.</p>
    </div>
  </div>
</section>
